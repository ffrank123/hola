...
use App\Models\Service; // <-- Aquí se importa el modelo Service
...
class PromotionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['service_ids'])) {
            // Filtramos los servicios activos y con capacidad disponible
            $activeServices = Service::whereIn('id', $data['service_ids'])
                ->where('status', 'active')
                ->where('capacity', '>', 0) // Solo servicios con capacidad disponible
                ->get();
    
            $promotion->services()->sync($activeServices);
        }
